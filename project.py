...
if uploaded_file is not None:
    ...
    if test_type == "Chi-Square":
        ...
        col = st.selectbox("Select numerical variable", encoded_data.columns)
        groups = [group for name, group in encoded_data.groupby(col)]
        if len(groups) >= 2:
            f_stat, p = stats.f_oneway(*groups)
            st.write(f"ANOVA Test: p-value = {p}")
            if p < 0.05:
                st.write("**Result:** Reject null hypothesis (Significant variance between groups)")
            else:
                st.write("**Result:** Fail to reject null hypothesis (No significant variance)")
        else:
            st.write("Not enough groups for ANOVA test.")
